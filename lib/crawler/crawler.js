...
NSCrawler.prototype.explore = function(source) {
  ...
  node.checkDigest(this.config.platform, source).then(() => {
    ...
    node.raw = this.config.platform === 'pc-web' ? JSON.stringify(source.value) : source.value;
...
NSCrawler.prototype.recursiveFilter = function (source, matches, exclusive) {
  ...
  for (let i = 0; i < this.config.asserts.length; i++) {
    if (this.checkContentMatch(source, this.config.asserts[i].given, true)) {
              ...
              .then(data => {
                if (data.status === 0) {
                  if (this.config.clickTypes.indexOf(action.source.type) >= 0) {
                    ...
                    root.wdclient.send('/wd/hub/session/' + this.sessionId + '/element/' + data.value.ELEMENT + '/click', 'post', {}, () => {
                      this.refreshScreen();
                    });
...
NSCrawler.prototype.insertTabNode = function (rawElement) {
  if (!hooks.insertTabNode(rawElement, this)) {
    ...
    let node = new NSAppCrawlingTreeNode();
    node.actions = node.produceNodeActions(rawElement);
    node.type = 'tab';
